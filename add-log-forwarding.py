###############################################################################
#
# Script:       add-log-forwarding.py
#
# Author:       Chris Goodwin <chrisgoodwins@gmail.com>
#
# Description:  This script presents the user with the ability to apply log
#               forwarding profiles to selected policies, or all policies
#               within a device group. Once the user authenticates to Panorama,
#               the user is presented with a list of device groups. Then, the
#               user chooses from a list of log forwarding profiles contained
...
def getParentDGs(devGroup, baseurl, mainkey):
    ...
    sharedLogProfURL = baseurl + '/api/?type=config&action=get&xpath=/config/shared/log-settings/profiles&key=' + mainkey
    r = requests.get(sharedLogProfURL, verify=False)
    sharedLogProfTree = ET.fromstring(r.text)
    if sharedLogProfTree.find('./result/profiles/entry') is not None:
        pDGs.append('shared')
    return pDGs


# Presents the user with a choice of log forwarding profiles for the chosen device-group
def getLogProfile(devGroup, baseurl, mainkey):
    allDGs = getParentDGs(devGroup, baseurl, mainkey)
    allDGs.append(devGroup)
    logProfileList = []
    for dg in allDGs:
        if dg == 'shared':
            logProfileXmlUrl = "/api/?type=config&action=get&xpath=/config/shared/log-settings/profiles&key="
        else:
            logProfileXmlUrl = "/api/?type=config&action=get&xpath=/config/devices/entry[@name='localhost.localdomain']/device-group/entry[@name='%s']/log-settings/profiles&key=" % (dg)
        logProfileFullUrl = (baseurl + logProfileXmlUrl + mainkey)
        r = requests.get(logProfileFullUrl, verify=False)
        logProfileFwTree = ET.fromstring(r.text)
        for entry in logProfileFwTree.findall('./result/profiles/entry'):
            logProfileList.append(entry.get('name'))
    if logProfileList != []:
        while True:
            try:
                print('\n\n\nHere\'s a list of log forwarding profiles found for the %s device group...\n' % (devGroup))
                i = 1
                for LogProfileName in logProfileList:
                    print('%s) %s' % (i, LogProfileName))
                    i += 1
                LogProfileChoice = int(input('\nChoose a number for the log forwarding profile\n\nAnswer: '))
                reportLogProfile = logProfileList[LogProfileChoice - 1]
                break
            except:
                print("\n\nThat's not a number in the list, try again...\n")
    else:
        print('\n\nThere were no log forwarding profiles found for the %s device group, please choose another device group...\n' % (devGroup))
        devGroup = getDG(baseurl, mainkey)
    return reportLogProfile
...
def getPolicies(devGroup, baseurl, mainkey):
    ...
    while True:
        ...
        secPolicyList_answer = input("Type 'all' to apply the the log forwarding profile to all security policies,\nor hit enter to display the policies in order to select which to apply the profile\n\nAnswer: ")
        if secPolicyList_answer == 'all':
...
# Applies the log forwarding profile to the selected rulebase and device group
def main():
    fwip = getfwipfqdn()
    mainkey = getkey(fwip)
    baseurl = 'https://' + fwip
    ...
    while True:
        devGroup = getDG(baseurl, mainkey)
        logProfile = getLogProfile(devGroup, baseurl, mainkey)
        ...
        while run:
            time.sleep(1)
            applyProfile_answer = input('\n\nWould you like to go ahead with applying the %s log forwarding profile to the policies specified above for the %s device group? [y/n]\n\nAnswer: ' % (logProfile, devGroup))
            if applyProfile_answer == 'y' or applyProfile_answer == 'Y':
                xmlElement = ''
                count = 0
                for secPolicy in secPolicies:
                    xmlElement = xmlElement + "<entry name='%s'><log-setting>%s</log-setting></entry>" % (secPolicy, logProfile)
                    count += 1
                applyProfilesURL = baseurl + "/api/?type=config&action=set&xpath=/config/devices/entry[@name='localhost.localdomain']/device-group/entry[@name='" + devGroup + "']/" + rulebaseType + "/security/rules&element=" + xmlElement + '&key=' + mainkey
                ...
                if applyProfilesFwTree.get('status') == 'success':
                    time.sleep(1)
                    print("\n\nCongrats! The '%s' log forwarding profile was applied to the %s specified policies in the %s device group\n\n\n\nHave a great day!!!\n\n" % (logProfile, count, devGroup))
                    exit()
                else:
